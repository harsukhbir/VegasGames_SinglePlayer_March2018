...
(function(global) {
  ...
  AppsFlyer.prototype.onInstallConversionDataLoaded = function(conversionData) {
    ...
    event = new CustomEvent('onInstallConversionDataLoaded', {
      'detail': data
    });
...
document.addEventListener("deviceready", function() {
  ...
  var args = [];
  var devKey = "qd4YhrYeW2v5CSAAXTSoY7"; // your AppsFlyer devKey
  args.push(devKey);
  var userAgent = window.navigator.userAgent.toLowerCase();

  if (/iphone|ipad|ipod/.test(userAgent)) {
    var appId = "1077181944"; // your ios app id in app store
    args.push(appId);
  }

  window.plugins.appsFlyer.initSdk(args);
  window.plugins.appsFlyer.trackEvent(Vegas_Games_Sessions, Vegas_Games_Sessions_No);
  // window.plugins.appsFlyer.trackEvent(Open_From_PushNotification, Open_From_PushNotification_No);
  // window.plugins.appsFlyer.trackEvent(Vegas_Games_Rate, Vegas_Games_Rate_No);
  window.plugins.appsFlyer.trackEvent(Level_Achieved, Level_Achieved_No);
  var userId = "io.vegasgames.singleplayer"
